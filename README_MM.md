...
| Syntax highlighting | comment၊ string၊ number၊ declaration၊ keyword၊ type၊ constant၊ operator၊ canonical `say` output command နှင့် `spawn`၊ `task_join`၊ `task_is_ready` async task builtin များကို highlight ပြခြင်း |
